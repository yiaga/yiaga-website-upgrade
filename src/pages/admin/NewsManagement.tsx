...
const NewsManagement = () => {
  ...
  const [editingNews, setEditingNews] = useState<typeof newsItems[0] | null>(null);
  ...
  const handleCreate = () => {
    addAuditLog('CREATE_NEWS', `Created news article: ${formData.title}`);
    toast({
      title: 'News article created',
      description: 'The news article has been created successfully (demo mode).',
    });
    setIsCreateOpen(false);
    resetForm();
  };

  const handleEdit = (article: typeof newsItems[0]) => {
  ...
  const handleUpdate = () => {
    addAuditLog('UPDATE_NEWS', `Updated news article: ${formData.title}`);
    toast({
      title: 'News article updated',
      description: 'The news article has been updated successfully (demo mode).',
    });
    setEditingNews(null);
    resetForm();
  };

  const handleDelete = (article: typeof newsItems[0]) => {
    addAuditLog('DELETE_NEWS', `Deleted news article: ${article.title}`);
    toast({
      title: 'News article deleted',
      description: 'The news article has been deleted successfully (demo mode).',
      variant: 'destructive',
    });
  };
